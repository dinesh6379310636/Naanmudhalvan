...
# Function to download dataset directly using Kaggle API endpoint
def load_kaggle_dataset():
    try:
        ...
        if not kaggle_username or not kaggle_key:
            return "Error: KAGGLE_USERNAME and KAGGLE_KEY environment variables must be set."
        
        # Kaggle dataset details
        dataset_owner = "kashishparmar02"
        dataset_name = "social-media-sentiments-analysis-dataset"
        dataset_url = f"https://www.kaggle.com/api/v1/datasets/download/{dataset_owner}/{dataset_name}"
        ...
        if response.status_code != 200:
            return f"Error: Failed to download dataset. Status code: {response.status_code}"
        
        # The dataset is downloaded as a ZIP file; extract it
        ...
        if not os.path.exists(csv_file):
            return "Error: 'sentimentdataset.csv' not found in the dataset."
        ...
        # Rename 'Text' to 'text' for consistency with the rest of the code
...
# New Gradio interface with card-based layout
with gr.Blocks(title="Emotion Detection in Social Media Conversations", theme=theme) as demo:
    ...
    with gr.Group():
        ...
        gr.Markdown(
            "Preview the dataset sourced from Kaggle to see sample posts and their labeled sentiments.",
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            csv_button = gr.Button("Analyze File", variant="primary")
        ...
        csv_button.click(fn=analyze_csv, inputs=csv_input, outputs=csv_output)
    ...
    gr.Markdown(
        ...
        Dataset: [Social Media Sentiments Analysis Dataset](https://www.kaggle.com/datasets/kashishparmar02/social-media-sentiments-analysis-dataset)  
        Model: `j-hartmann/emotion-english-distilroberta-base`  
        """
    )
